...
import type { WhitelistedUser } from '@/api/opa'

interface Props {
  user: WhitelistedUser | null
  open: boolean
  onClose: () => void
  onSave: (user: WhitelistedUser) => void
}

export function WhitelistedUserModal({ user, open, onClose, onSave }: Props) {
  const [draft, setDraft] = useState<WhitelistedUser>(empty())
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md bg-surface border-border text-text">
        <DialogHeader>
          <DialogTitle>{user ? 'Edit Whitelist Entry' : 'Add Whitelisted User'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <div className="min-h-10 p-1 rounded-[5px] border border-border bg-card flex flex-wrap gap-1 items-center">
              {draft.hosts.map(h => (
                <span key={h} className="bg-surface border border-border px-2 py-0.5 rounded-[3px] text-[12px] font-mono text-green flex items-center gap-1.5">
                  {h}
            ...
            <p className="text-[11px] text-text-dim italic">Leave empty to whitelist across all hosts.</p>
          </div>
        </div>
        ...
        <DialogFooter className="mt-4 border-t border-border pt-4">
          ...
          <Button onClick={save} className="h-9 px-6 bg-green hover:bg-green/90 text-black font-bold">Save</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
...
function empty(): WhitelistedUser {
