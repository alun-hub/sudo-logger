...
echo "  mkdir -p /etc/systemd/system/sudo-logger-agent.service.d/"
echo "  cat > /etc/systemd/system/sudo-logger-agent.service.d/server.conf << 'EOF'"
echo "  [Service]"
echo "  ExecStart="
echo "  ExecStart=/usr/bin/sudo-logger-agent -server DIN-SERVER:9876 \\"
