...
## Core Workflow
1. **Automatic Version Bump**: The build script automatically increments the version in `rpm/*.spec`.
2. **Auto-Commit**: The version change is automatically committed to git so it is included in the build.
3. **Archive**: Create the source tarball in `~/rpmbuild/SOURCES/` using `git archive`.
